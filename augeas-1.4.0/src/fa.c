...
//int fa_minimization_algorithm = FA_MIN_BRZOZOWSKI;
...
/* Used to keep autstate of the regex parse; RX may contain NUL's */
struct re_parse {
...
/* A map from a set of autstates to a autstate. */
typedef hash_t autstate_set_hash;
...
struct autstate_set_list {
    struct autstate_set_list *next;
    struct autstate_set      *set;
};

/* Clean up FA by removing dead transitions and autstates and reducing
 * transitions. Unreachable autstates are freed. The return value is the same
 * as FA; returning it is merely a convenience.
 *
 * Only automata in this autstate should be returned to the user
 */
...
static void free_trans(struct autstate *s) {
    free(s->trans);
    s->trans = NULL;
    s->tused = s->tsize = 0;
}
...
static struct autstate *make_autstate(void) {
    struct autstate *s;
    if (ALLOC(s) == -1)
        return NULL;
    s->hash = ptr_hash(s);
...
struct autstate *add_autstate(struct fa *fa, int accept) {
    struct autstate *s = make_autstate();
    if (s) {
        s->accept = accept;
        if (fa->initial == NULL) {
            fa->initial = s;
...
ATTRIBUTE_RETURN_CHECK
int add_epsilon_trans(struct autstate *from,
                             struct autstate *to) {
    int r;
    from->accept |= to->accept;
    for_each_trans(t, to) {
        r = add_new_auttrans(from, t->to, t->min, t->max);
        if (r < 0)
            return -1;
    }
    return 0;
...
/* Merge automaton FA2 into FA1. This simply adds FA2's autstates to FA1
...
/*
 * Operations on autstate_SET
 */
static void autstate_set_free(struct autstate_set *set) {
    if (set == NULL)
        return;
    free(set->autstates);
    free(set->data);
...
static int autstate_set_init_data(struct autstate_set *set) {
    set->with_data = 1;
    if (set->data == NULL)
...
/* Create a new autstate_SET with an initial size of SIZE. If SIZE is -1, use
   the default size ARRAY_INITIAL_SIZE. FLAGS is a bitmask indicating
   some options:
   - S_SORTED: keep the autstates in the set sorted by their address, and use
   ...
   - S_DATA: allocate the DATA array in the set, and keep its size in sync
     with the size of the autstateS array.
*/
static struct autstate_set *autstate_set_init(int size, int flags) {
    struct autstate_set *set = NULL;
    ...
    if (size > 0) {
        set->size = size;
        F(ALLOC_N(set->autstates, set->size));
        if (set->with_data)
            F(autstate_set_init_data(set));
    }
    return set;
 error:
    autstate_set_free(set);
    return NULL;
}

ATTRIBUTE_RETURN_CHECK
static int autstate_set_expand(struct autstate_set *set) {
    if (set->size == 0)
        ...
        set->size *= 2;
    if (REALLOC_N(set->autstates, set->size) < 0)
        goto error;
    if (set->with_data)
...
/* Return the index where S belongs in SET->autstateS to keep it sorted.  S
   may not be in SET->autstateS. The returned index is in the interval [0
   .. SET->USED], with the latter indicating that S is larger than all
   values in SET->autstateS
*/
static int autstate_set_pos(const struct autstate_set *set, const struct autstate *s) {
    int l = 0, h = set->used;
    while (l < h) {
        int m = (l + h)/2;
        if (set->autstates[m] > s)
            h = m;
        else if (set->autstates[m] < s)
            l = m + 1;
        else
            return m;
    }
...
ATTRIBUTE_RETURN_CHECK
static int autstate_set_push(struct autstate_set *set, struct autstate *s) {
    if (set->size == set->used)
        if (autstate_set_expand(set) < 0)
            return -1;
    if (set->sorted) {
        int p = autstate_set_pos(set, s);
        if (set->size == set->used)
            if (autstate_set_expand(set) < 0)
                return -1;
        while (p < set->used && set->autstates[p] <= s)
            p += 1;
        if (p < set->used) {
            memmove(set->autstates + p + 1, set->autstates + p,
                    sizeof(*set->autstates) * (set->used - p));
            if (set->data != NULL)
        ...
        set->autstates[set->used++] = s;
        return set->used - 1;
    }
}

ATTRIBUTE_RETURN_CHECK
static int autstate_set_push_data(struct autstate_set *set, struct autstate *s,
                               void *d) {
    int i = autstate_set_push(set, s);
    if (i == -1)
        return -1;
    set->data[i] = d;
    return i;
...
static int autstate_set_index(const struct autstate_set *set,
                           const struct autstate *s) {
    if (set->sorted) {
        int p = autstate_set_pos(set, s);
        return (p < set->used && set->autstates[p] == s) ? p : -1;
    } else {
        for (int i=0; i < set->used; i++) {
            if (set->autstates[i] == s)
                return i;
        }
    }
    return -1;
...
static void autstate_set_remove(struct autstate_set *set,
                             const struct autstate *s) {
   if (set->sorted) {
       int p = autstate_set_index(set, s);
       if (p == -1) return;
       memmove(set->autstates + p, set->autstates + p + 1,
               sizeof(*set->autstates) * (set->used - p - 1));
       if (set->data != NULL)
       ...
       int p = autstate_set_index(set, s);
       if (p >= 0) {
           set->autstates[p] = set->autstates[--set->used];
       }
   }
}
...
ATTRIBUTE_RETURN_CHECK
static int autstate_set_add(struct autstate_set *set, struct autstate *s) {
    if (set->sorted) {
        int p = autstate_set_pos(set, s);
        if (p < set->used && set->autstates[p] == s)
            return 0;
        if (set->size == set->used)
            if (autstate_set_expand(set) < 0)
                return -1;
        ...
        if (autstate_set_index(set, s) >= 0)
            return 0;
        if (autstate_set_push(set, s) < 0)
            goto error;
    }
    return 1;
 error:
...
static struct autstate *autstate_set_pop(struct autstate_set *set) {
    struct autstate *s = NULL;
    if (set->used > 0)
        s = set->autstates[--set->used];
    return s;
}

static struct autstate *autstate_set_pop_data(struct autstate_set *set, void **d) {
    struct autstate *s = NULL;
    s = autstate_set_pop(set);
    *d = set->data[set->used];
    return s;
}

static void *autstate_set_find_data(struct autstate_set *set, struct autstate *s) {
    int i = autstate_set_index(set, s);
    if (i >= 0)
        return set->data[i];
    else
        return NULL;
...
static int autstate_set_equal(const struct autstate_set *set1,
                           const struct autstate_set *set2) {
    if (set1->used != set2->used)
        return 0;
    if (set1->sorted && set2->sorted) {
        for (int i = 0; i < set1->used; i++)
            if (set1->autstates[i] != set2->autstates[i])
                return 0;
        return 1;
    } else {
        for (int i=0; i < set1->used; i++)
            if (autstate_set_index(set2, set1->autstates[i]) == -1)
                return 0;
        return 1;
    }
}
...
#if 0
static void autstate_set_compact(struct autstate_set *set) {
    while (set->used > 0 && set->autstates[set->used] == NULL)
        set->used -= 1;
    for (int i=0; i < set->used; i++) {
        if (set->autstates[i] == NULL) {
            set->used -= 1;
            set->autstates[i] = set->autstates[set->used];
            if (set->data)
...
/* Add an entry (FST, SND) to SET. FST is stored in SET->autstateS, and SND is
   stored in SET->DATA at the same index.
*/
ATTRIBUTE_RETURN_CHECK
static int autstate_pair_push(struct autstate_set **set,
                           struct autstate *fst, struct autstate *snd) {
    if (*set == NULL)
        *set = autstate_set_init(-1, S_DATA);
    if (*set == NULL)
        return -1;
    int i = autstate_set_push(*set, fst);
    if (i == -1)
        return -1;
    (*set)->data[i] = snd;
...
/* Return the index of the pair (FST, SND) in SET, or -1 if SET contains no
   such pair.
 */
static int autstate_pair_find(struct autstate_set *set, struct autstate *fst,
                           struct autstate *snd) {
    for (int i=0; i < set->used; i++)
        if (set->autstates[i] == fst && set->data[i] == snd)
            return i;
    return -1;
}
...
typedef hash_t autstate_triple_hash;

static hash_val_t pair_hash(const void *key) {
    register struct autstate *const *pair = key;
    return pair[0]->hash + pair[1]->hash;
}

static int pair_cmp(const void *key1, const void *key2) {
    return memcmp(key1, key2, 2*sizeof(struct autstate *));
}

static void autstate_triple_node_free(hnode_t *node, ATTRIBUTE_UNUSED void *ctx) {
    free((void *) hnode_getkey(node));
    free(node);
}

static autstate_triple_hash *autstate_triple_init(void) {
    autstate_triple_hash *hash;

    hash = hash_create(HASHCOUNT_T_MAX, pair_cmp, pair_hash);
    if (hash == NULL)
        return NULL;
    hash_set_allocator(hash, NULL, autstate_triple_node_free, NULL);
    return hash;
}

ATTRIBUTE_RETURN_CHECK
static int autstate_triple_push(autstate_triple_hash *hash,
                             struct autstate *s1,
                             struct autstate *s2,
                             struct autstate *s3) {
    struct autstate **pair;
    if (ALLOC_N(pair, 2) < 0)
        return -1;
    pair[0] = s1;
    pair[1] = s2;
...
static struct autstate * autstate_triple_thd(autstate_triple_hash *hash,
                                       struct autstate *s1,
                                       struct autstate *s2) {
    struct autstate *pair[2];
    hnode_t *node;
    pair[0] = s1;
    pair[1] = s2;
    node = hash_lookup(hash, pair);
    return (node == NULL) ? NULL : (struct autstate *) hnode_get(node);
}

static void autstate_triple_free(autstate_triple_hash *hash) {
    if (hash != NULL) {
...
static int mark_reachable(struct fa *fa) {
    struct autstate_set *worklist = autstate_set_init(-1, S_NONE);
    int result = -1;
    ...
    for (struct autstate *s = fa->initial;
         s != NULL;
         s = autstate_set_pop(worklist)) {
        for_each_trans(t, s) {
            if (! t->to->reachable) {
                t->to->reachable = 1;
                F(autstate_set_push(worklist, t->to));
            }
        }
    }
    result = 0;
 ...
 error:
    autstate_set_free(worklist);
    return result;
}

/* Return all reachable autstates. As a sideeffect, all autstates have their
   REACHABLE flag set appropriately.
 */
static struct autstate_set *fa_autstates(struct fa *fa) {
    struct autstate_set *visited = autstate_set_init(-1, S_NONE);
    int r;
    ...
    list_for_each(s, fa->initial) {
        if (s->reachable)
            F(autstate_set_push(visited, s));
    }
    return visited;
 error:
    autstate_set_free(visited);
    return NULL;
}

/* Return all reachable accepting autstates. As a sideeffect, all autstates have
   their REACHABLE flag set appropriately.
 */
static struct autstate_set *fa_accept_autstates(struct fa *fa) {
    struct autstate_set *accept = autstate_set_init(-1, S_NONE);
    int r;
    ...
    list_for_each(s, fa->initial) {
        if (s->reachable && s->accept)
            F(autstate_set_push(accept, s));
    }
    return accept;
 error:
    autstate_set_free(accept);
    return NULL;
}

/* Mark all live autstates, i.e. autstates from which an accepting autstate can be
   reached. All autstates have their REACHABLE and LIVE flags set
   appropriately.
 */
ATTRIBUTE_RETURN_CHECK
static int mark_live(struct fa *fa) {
 ...
 //create a new state in fai..
 //
 //add epsilong transition from this initial state to each one in inits
 //
 //return 
 ...
 *
 * Returns a list of the new initial autstates of the automaton. The list must
 * be freed by the caller.
 */
static struct autstate_set *fa_reverse(struct fa *fa) {
    struct autstate_set *all = NULL;
    struct autstate_set *accept = NULL;
    int r;

    all = fa_autstates(fa);
    E(all == NULL);
    accept = fa_accept_autstates(fa);
    E(accept == NULL);

    F(autstate_set_init_data(all));

    /* Reverse all transitions */
    int *tused;
    F(ALLOC_N(tused, all->used));
    for (int i=0; i < all->used; i++) {
        all->data[i] = all->autstates[i]->trans;
        tused[i] = all->autstates[i]->tused;
        all->autstates[i]->trans = NULL;
        all->autstates[i]->tsize = 0;
        all->autstates[i]->tused = 0;
    }
    for (int i=0; i < all->used; i++) {
        struct autstate *s = all->autstates[i];
        struct auttrans *t = all->data[i];
        s->accept = 0;
        for (int j=0; j < tused[i]; j++) {
            r = add_new_auttrans(t[j].to, s, t[j].min, t[j].max);
            if (r < 0)
                goto error;
        }
        free(t);
    ...
    /* Make new initial and final autstates */
    struct autstate *s = add_autstate(fa, 0);
    E(s == NULL);

    fa->initial->accept = 1;
    set_initial(fa, s);
    for (int i=0; i < accept->used; i++) {
        r = add_epsilon_trans(s, accept->autstates[i]);
        if (r < 0)
            goto error;
    }
...
/*
 * Operations on autstate_SET_HASH
 */
static int autstate_set_hash_contains(autstate_set_hash *smap,
                               struct autstate_set *set) {
    return hash_lookup(smap, set) != NULL;
}

/*
 * Find the set in SMAP that has the same autstates as SET. If the two are
 * different, i.e. they point to different memory locations, free SET and
 * return the set found in SMAP
 */
static struct autstate_set *autstate_set_hash_uniq(autstate_set_hash *smap,
                                             struct autstate_set *set) {
    hnode_t *node = hash_lookup(smap, set);
    const struct autstate_set *orig_set = hnode_getkey(node);
    if (orig_set != set) {
        autstate_set_free(set);
    }
    return (struct autstate_set *) orig_set;
}

static struct autstate *autstate_set_hash_get_autstate(autstate_set_hash *smap,
                                             struct autstate_set *set) {
    hnode_t *node = hash_lookup(smap, set);
    return (struct autstate *) hnode_get(node);
}

static hash_val_t set_hash(const void *key) {
    hash_val_t hash = 0;
    const struct autstate_set *set = key;

    for (int i = 0; i < set->used; i++) {
        hash += set->autstates[i]->hash;
    }
    return hash;
}

static int set_cmp(const void *key1, const void *key2) {
    const struct autstate_set *set1 = key1;
    const struct autstate_set *set2 = key2;

    return autstate_set_equal(set1, set2) ? 0 : 1;
}

static void set_destroy(hnode_t *node, ATTRIBUTE_UNUSED void *ctx) {
    struct autstate_set *set = (struct autstate_set *) hnode_getkey(node);
    autstate_set_free(set);
    free(node);
}

ATTRIBUTE_RETURN_CHECK
static int autstate_set_hash_add(autstate_set_hash **smap,
                              struct autstate_set *set, struct fa *fa) {
    if (*smap == NULL) {
...
static void autstate_set_hash_free(autstate_set_hash *smap,
                                struct autstate_set *protect) {
    if (protect != NULL) {
...
static int autstate_set_list_add(struct autstate_set_list **list,
                              struct autstate_set *set) {
    struct autstate_set_list *elt;
    if (ALLOC(elt) < 0)
        return -1;
    elt->set = set;
    list_cons(*list, elt);
...
static struct autstate_set *autstate_set_list_pop(struct autstate_set_list **list) {
    struct autstate_set_list *elt = *list;
    struct autstate_set      *set = elt->set;
...
static int trans_to_cmp(const void *v1, const void *v2) {
    const struct auttrans *t1 = v1;
    const struct auttrans *t2 = v2;
 ...
 * lead to a live autstate. This also removes any autstates that are not
...
static void collect_dead_autstates(struct fa *fa) {
    /* Remove all dead autstates and free their storage */
    for (struct autstate *s = fa->initial; s->next != NULL; ) {
        if (! s->next->live) {
            struct autstate *del = s->next;
            s->next = del->next;
            free_trans(del);
            free(del);
        } else {
 ...
 * autstates with the subset construction. This also eliminates dead autstates
 * and transitions and reduces and orders the transitions for each autstate
 */
static int determinize(struct fa *fa, struct autstate_set *ini) {
    int npoints;
    int make_ini = (ini == NULL);
    const uchar *points = NULL;
    autstate_set_hash *newautstate = NULL;
    struct autstate_set_list *worklist = NULL;
    int ret = 0;
    ...
    E(points == NULL);
    if (make_ini) {
        ini = autstate_set_init(-1, S_NONE);
        if (ini == NULL || autstate_set_push(ini, fa->initial) < 0)
            goto error;
    }

    F(autstate_set_list_add(&worklist, ini));
    F(autstate_set_hash_add(&newautstate, ini, fa));
    // Make the new autstate the initial autstate
    swap_initial(fa);
    while (worklist != NULL) {
        struct autstate_set *sset = autstate_set_list_pop(&worklist);
        struct autstate *r = autstate_set_hash_get_autstate(newautstate, sset);
        for (int q=0; q < sset->used; q++) {
            r->accept |= sset->autstates[q]->accept;
        }
        for (int n=0; n < npoints; n++) {
            struct autstate_set *pset = autstate_set_init(-1, S_SORTED);
            E(pset == NULL);
            for(int q=0 ; q < sset->used; q++) {
                for_each_trans(t, sset->autstates[q]) {
                    if (t->min <= points[n] && points[n] <= t->max) {
                        F(autstate_set_add(pset, t->to));
                    }
                }
            }
            if (!autstate_set_hash_contains(newautstate, pset)) {
                F(autstate_set_list_add(&worklist, pset));
                F(autstate_set_hash_add(&newautstate, pset, fa));
            }
            pset = autstate_set_hash_uniq(newautstate, pset);

            struct autstate *q = autstate_set_hash_get_autstate(newautstate, pset);
            uchar min = points[n];
            uchar max = UCHAR_MAX;
            if (n+1 < npoints)
                max = points[n+1] - 1;
            if (add_new_auttrans(r, q, min, max) < 0)
                goto error;
        }
    }
    fa->deterministic = 1;
    ...
    if (newautstate)
        autstate_set_hash_free(newautstate, make_ini ? NULL : ini);
    free((void *) points);
    if (collect(fa) < 0)
        ret = -1;
    return ret;
...
int determinizee(struct fa* fa){
    struct autstate_set *set = autstate_set_init(-1, S_DATA|S_SORTED);
    autstate_set_push(set,fa->initial);
    int res = determinize(fa,set);
    return res;
}
...
static struct autstate *step(struct autstate *s, uchar c) {
    for_each_trans(t, s) {
        if (t->min <= c && c <= t->max)
            return t->to;
    }
...
static struct autstate* recRun(struct autstate* st, char* run, int curindex, int length){
...
struct autstate_list {
    struct autstate_list_node *first;
    struct autstate_list_node *last;
    unsigned int            size;
};

struct autstate_list_node {
    struct autstate_list      *sl;
    struct autstate_list_node *next;
    struct autstate_list_node *prev;
    struct autstate           *autstate;
};

static struct autstate_list_node *autstate_list_add(struct autstate_list *sl,
                                              struct autstate *s) {
    struct autstate_list_node *n;
...
static void autstate_list_remove(struct autstate_list_node *n) {
    struct autstate_list *sl = n->sl;
    sl->size -= 1;
    if (sl->first == n)
        sl->first = n->next;
    else
...
static void autstate_list_free(struct autstate_list *sl) {
    if (sl)
        list_free(sl->first);
    free(sl);
}

/* The linear index of element (q,c) in an NautstateS * NSIGMA matrix */
#define INDEX(q, c) (q * nsigma + c)

static int minimize_hopcroft(struct fa *fa) {
    struct autstate_set *autstates = NULL;
    uchar *sigma = NULL;
    struct autstate_set **reverse = NULL;
    bitset *reverse_nonempty = NULL;
    struct autstate_set **partition = NULL;
    unsigned int *block = NULL;
    struct autstate_list **active = NULL;
    struct autstate_list_node **active2 = NULL;
    int *pending = NULL;
    bitset *pending2 = NULL;
    struct autstate_set *split = NULL;
    bitset *split2 = NULL;
    int *refine = NULL;
    bitset *refine2 = NULL;
    struct autstate_set **splitblock = NULL;
    struct autstate_set *newautstates = NULL;
    int *nsnum = NULL;
    int *nsind = NULL;
    int result = -1;
    ...
    /* make arrays for numbered autstates and effective alphabet */
    autstates = autstate_set_init(-1, S_NONE);
    E(autstates == NULL);

    list_for_each(s, fa->initial) {
        F(autstate_set_push(autstates, s));
    }
    unsigned int nautstates = autstates->used;
    ...
    /* An ss->used x nsigma matrix of lists of autstates */
    F(ALLOC_N(reverse, nautstates * nsigma));
    reverse_nonempty = bitset_init(nautstates * nsigma);
    E(reverse_nonempty == NULL);
    F(ALLOC_N(partition, nautstates));
    F(ALLOC_N(block, nautstates));

    F(ALLOC_N(active, nautstates * nsigma));
    F(ALLOC_N(active2, nautstates * nsigma));
    ...
    pending2 = bitset_init(nautstates * nsigma);
    E(pending2 == NULL);

    split = autstate_set_init(-1, S_NONE);
    split2 = bitset_init(nautstates);
    E(split == NULL || split2 == NULL);

    F(ALLOC_N(refine, nautstates));
    refine2 = bitset_init(nautstates);
    E(refine2 == NULL);

    F(ALLOC_N(splitblock, nautstates));

    for (int q = 0; q < nautstates; q++) {
        splitblock[q] = autstate_set_init(-1, S_NONE);
        partition[q] = autstate_set_init(-1, S_NONE);
        E(splitblock[q] == NULL || partition[q] == NULL);
        for (int x = 0; x < nsigma; x++) {
            reverse[INDEX(q, x)] = autstate_set_init(-1, S_NONE);
    ...
    for (int q = 0; q < nautstates; q++) {
        struct autstate *qq = autstates->autstates[q];
        int j;
        if (qq->accept)
            j = 0;
        else
            j = 1;
        F(autstate_set_push(partition[j], qq));
        block[q] = j;
        for (int x = 0; x < nsigma; x++) {
            uchar y = sigma[x];
            struct autstate *p = step(qq, y);
            assert(p != NULL);
            int pn = autstate_set_index(autstates, p);
            assert(pn >= 0);
            F(autstate_set_push(reverse[INDEX(pn, x)], qq));
    ...
    for (int j = 0; j <= 1; j++)
        for (int x = 0; x < nsigma; x++)
            for (int q = 0; q < partition[j]->used; q++) {
                struct autstate *qq = partition[j]->autstates[q];
                int qn = autstate_set_index(autstates, qq);
                if (bitset_get(reverse_nonempty, INDEX(qn, x))) {
                    active2[INDEX(qn, x)] =
                        autstate_list_add(active[INDEX(j, x)], qq);
    ...
    while (npending-- > 0) {
        ...
        int ref = 0;
        /* find autstates that need to be split off their blocks */
        struct autstate_list *sh = active[INDEX(p,x)];
        for (struct autstate_list_node *m = sh->first; m != NULL; m = m->next) {
            int q = autstate_set_index(autstates, m->autstate);
            struct autstate_set *rev = reverse[INDEX(q, x)];
            for (int r =0; r < rev->used; r++) {
                struct autstate *rs = rev->autstates[r];
                int s = autstate_set_index(autstates, rs);
                if (! bitset_get(split2, s)) {
                    bitset_set(split2, s);
                    F(autstate_set_push(split, rs));
                    int j = block[s];
                    F(autstate_set_push(splitblock[j], rs));
        ...
        for(int rr=0; rr < ref; rr++) {
            int j = refine[rr];
            struct autstate_set *sp = splitblock[j];
            if (sp->used < partition[j]->used) {
                struct autstate_set *b1 = partition[j];
                struct autstate_set *b2 = partition[k];
                for (int s = 0; s < sp->used; s++) {
                    autstate_set_remove(b1, sp->autstates[s]);
                    F(autstate_set_push(b2, sp->autstates[s]));
                    int snum = autstate_set_index(autstates, sp->autstates[s]);
                    block[snum] = k;
                    for (int c = 0; c < nsigma; c++) {
                        struct autstate_list_node *sn = active2[INDEX(snum, c)];
                        if (sn != NULL && sn->sl == active[INDEX(j,c)]) {
                            autstate_list_remove(sn);
                            active2[INDEX(snum, c)] =
                                autstate_list_add(active[INDEX(k, c)],
                                               sp->autstates[s]);
    ...
    /* make a new autstate for each equivalence class, set initial autstate */
    newautstates = autstate_set_init(k, S_NONE);
    E(newautstates == NULL);
    F(ALLOC_N(nsnum, k));
    F(ALLOC_N(nsind, nautstates));

    for (int n = 0; n < k; n++) {
        struct autstate *s = make_autstate();
        E(s == NULL);
        newautstates->autstates[n] = s;
        struct autstate_set *partn = partition[n];
        for (int q=0; q < partn->used; q++) {
            struct autstate *qs = partn->autstates[q];
            int qnum = autstate_set_index(autstates, qs);
            if (qs == fa->initial)
                s->live = 1;     /* Abuse live to flag the new intial autstate */
            nsnum[n] = qnum;     /* select representative */
            nsind[qnum] = n;     /* and point from partition to new autstate */
        }
    }

    /* build transitions and set acceptance */
    for (int n = 0; n < k; n++) {
        struct autstate *s = newautstates->autstates[n];
        s->accept = autstates->autstates[nsnum[n]]->accept;
        for_each_trans(t, autstates->autstates[nsnum[n]]) {
            int toind = autstate_set_index(autstates, t->to);
            struct autstate *nto = newautstates->autstates[nsind[toind]];
            F(add_new_auttrans(s, nto, t->min, t->max));
        }
    }

    /* Get rid of old autstates and transitions and turn NEWTautstateS into
       a linked list */
    gut(fa);
    for (int n=0; n < k; n++)
        if (newautstates->autstates[n]->live) {
            struct autstate *ini = newautstates->autstates[n];
            newautstates->autstates[n] = newautstates->autstates[0];
            newautstates->autstates[0] = ini;
        }
    for (int n=0; n < k-1; n++)
        newautstates->autstates[n]->next = newautstates->autstates[n+1];
    fa->initial = newautstates->autstates[0];
    ...
    free(nsind);
    free(nsnum);
    autstate_set_free(autstates);
    free(sigma);
    bitset_free(reverse_nonempty);
    free(block);
    for (int i=0; i < nautstates*nsigma; i++) {
        if (reverse)
            autstate_set_free(reverse[i]);
        if (active)
            autstate_list_free(active[i]);
    }
    free(reverse);
    free(active);
    free(active2);
    ...
    autstate_set_free(split);
    bitset_free(split2);
    free(refine);
    bitset_free(refine2);
    for (int q=0; q < nautstates; q++) {
        if (splitblock)
            autstate_set_free(splitblock[q]);
        if (partition)
            autstate_set_free(partition[q]);
    }
    free(splitblock);
    free(partition);
    autstate_set_free(newautstates);
...
{
 struct autstate_set* set = fa_reverse(fai);
 E(set == NULL);
 F(determinize(fai, set));
 autstate_set_free(set);
 return 0;
error:
 return -1;
...
struct fa *fa_make_empty() {
    struct fa *fa;

    if (ALLOC(fa) < 0)
        return NULL;
    if (add_autstate(fa, 0) == NULL) {
        fa_free(fa);
        return NULL;
    }
    fa->deterministic = 1;
...
static struct fa *fa_make_char(uchar c) {
    ...
    struct autstate *s = fa->initial;
    struct autstate *t = add_autstate(fa, 1);
    int r;

    if (t == NULL)
        goto error;

    r = add_new_auttrans(s, t, c, c);
    if (r < 0)
        goto error;
    fa->deterministic = 1;
    fa->minimal = 1;
...
struct fa *fa_make_basic(unsigned int basic) {
    ...
    if (basic == FA_EMPTY) {
        ...
        r = add_new_auttrans(fa->initial, fa->initial, UCHAR_MIN, UCHAR_MAX);
        if (r < 0) {
            fa_free(fa);
            fa = NULL;
        }
    ...
    if (basic == FA_EMPTY) {
        ...
        if (fa->nocase) {
            if (fa->initial->tused != 2)
                return 0;
            struct auttrans *t1 = fa->initial->trans;
            struct auttrans *t2 = fa->initial->trans + 1;
            ...
            struct auttrans *t = fa->initial->trans;
...
struct fa *fa_clone(struct fa *fa) {
    ...
    list_for_each(s, fa->initial) {
        ...
        struct autstate *q = add_autstate(result, s->accept);
        if (q == NULL)
            goto error;
        set->data[i] = q;
        q->live = s->live;
    ...
    for (int i=0; i < set->used; i++) {
        struct autstate *s = set->autstates[i];
        struct autstate *sc = set->data[i];
        for_each_trans(t, s) {
            int to = autstate_set_index(set, t->to);
            assert(to >= 0);
            struct autstate *toc = set->data[to];
            r = add_new_auttrans(sc, toc, t->min, t->max);
            if (r < 0)
                goto error;
        }
    }
...
static struct fa *fa_make_char_set(bitset *cset, int negate) {
    ...
    while (from <= UCHAR_MAX) {
        ...
        while (to < UCHAR_MAX && (bitset_get(cset, to + 1) == !negate))
            to += 1;
        r = add_new_auttrans(s, t, from, to);
        if (r < 0)
            goto error;
        from = to + 1;
    }
...
struct fa *fa_intersect(struct fa *fa1, struct fa *fa2) {
    int ret;
    struct fa *fa = NULL;
    struct autstate_set *worklist = NULL;
    autstate_triple_hash *newautstates = NULL;
    ...
    fa = fa_make_empty();
    worklist = autstate_set_init(-1, S_NONE);
    newautstates = autstate_triple_init();
    if (fa == NULL || worklist == NULL || newautstates == NULL)
        goto error;

    sort_transition_intervals(fa1);
    sort_transition_intervals(fa2);

    F(autstate_set_push(worklist, fa1->initial));
    F(autstate_set_push(worklist, fa2->initial));
    F(autstate_set_push(worklist, fa->initial));
    F(autstate_triple_push(newautstates,
                         fa1->initial, fa2->initial, fa->initial));
    while (worklist->used) {
        struct autstate *s  = autstate_set_pop(worklist);
        struct autstate *p2 = autstate_set_pop(worklist);
        struct autstate *p1 = autstate_set_pop(worklist);
        s->accept = p1->accept && p2->accept;

        struct auttrans *t1 = p1->trans;
        struct auttrans *t2 = p2->trans;
        for (int n1 = 0, b2 = 0; n1 < p1->tused; n1++) {
            ...
            for (int n2 = b2;
                ...
                if (t2[n2].max >= t1[n1].min) {
                    struct autstate *r = autstate_triple_thd(newautstates,
                                                       t1[n1].to, t2[n2].to);
                    if (r == NULL) {
                        r = add_autstate(fa, 0);
                        E(r == NULL);
                        F(autstate_set_push(worklist, t1[n1].to));
                        F(autstate_set_push(worklist, t2[n2].to));
                        F(autstate_set_push(worklist, r));
                        F(autstate_triple_push(newautstates,
                                             t1[n1].to, t2[n2].to, r));
                    }
                    ...
                    char max = t1[n1].max < t2[n2].max
                        ? t1[n1].max : t2[n2].max;
                    ret = add_new_auttrans(s, r, min, max);
                    if (ret < 0)
                        goto error;
                }
            }
    ...
    autstate_set_free(worklist);
    autstate_triple_free(newautstates);
    if (fa != NULL) {
        if (collect(fa) < 0) {
            fa_free(fa);
            fa = NULL;
...
int fa_contains(struct fa *fa1, struct fa *fa2) {
    int result = 0;
    struct autstate_set *worklist = NULL;  /* List of pairs of autstates */
    struct autstate_set *visited = NULL;   /* List of pairs of autstates */
    ...
    F(autstate_pair_push(&worklist, fa1->initial, fa2->initial));
    F(autstate_pair_push(&visited, fa1->initial, fa2->initial));
    while (worklist->used) {
        struct autstate *p1, *p2;
        void *v2;
        p1 = autstate_set_pop_data(worklist, &v2);
        p2 = v2;
        ...
        for(int n1 = 0, b2 = 0; n1 < p1->tused; n1++) {
            ...
            for (int n2 = b2;
                ...
                if (autstate_pair_find(visited, t1[n1].to, t2[n2].to) == -1) {
                    F(autstate_pair_push(&worklist, t1[n1].to, t2[n2].to));
                    F(autstate_pair_push(&visited, t1[n1].to, t2[n2].to));
                }
            }
            if (min1 <= max1)
                goto done;
...
static int add_crash_trans(struct fa *fa, struct autstate *s, struct autstate *crash,
                           int min, int max) {
    int result;
    ...
    if (fa->nocase) {
        ...
        if (min > 'Z' || max < 'A') {
            result = add_new_auttrans(s, crash, min, max);
        } else if (min >= 'A' && max <= 'Z') {
            result = 0;
        } else if (max <= 'Z') {
            /* min < 'A' */
            result = add_new_auttrans(s, crash, min, 'A' - 1);
        } else if (min >= 'A') {
            /* max > 'Z' */
            result = add_new_auttrans(s, crash, 'Z' + 1, max);
        } else {
            /* min < 'A' && max > 'Z' */
            result = add_new_auttrans(s, crash, min, 'A' - 1);
            if (result == 0)
...
static int totalize(struct fa *fa) {
    int r;
    struct autstate *crash = add_autstate(fa, 0);
...
static char pick_char(struct auttrans *t) {
...
int fa_example_withendstate(struct fa *fa, char **example, size_t *example_len, struct autstate** acceptedstate) {
    ...
    struct autstate_set *path = NULL, *worklist = NULL;
    ...
    *example_len = 0;
    ...
    /* Map from autstate to string */
    path = autstate_set_init(-1, S_DATA|S_SORTED);
    str = make_re_str("");
    if (path == NULL || str == NULL)
        goto error;
    F(autstate_set_push_data(path, fa->initial, str));
    str = NULL;

    /* List of autstates still to visit */
    worklist = autstate_set_init(-1, S_NONE);
    if (worklist == NULL)
        goto error;
    F(autstate_set_push(worklist, fa->initial));
    ...
    while (worklist->used) {
        struct autstate *s = autstate_set_pop(worklist);
        struct re_str *ps = autstate_set_find_data(path, s);
        for_each_trans(t, s) {
            char c = pick_char(t);
            int toind = autstate_set_index(path, t->to);
            if (toind == -1) {
                struct re_str *w = string_extend(NULL, ps, c);
                E(w == NULL);
                F(autstate_set_push(worklist, t->to));
                F(autstate_set_push_data(path, t->to, w));
            } else {
    ...
    for (int i=0; i < path->used; i++) {
        struct autstate *p = path->autstates[i];
 ...
 * at each turn the "best" word found for that autstate.
 */
...
static int fa_enumerate_intl(struct autstate *s, struct enum_intl *ei, int pos) {
    int result = -1;
...
int fa_enumerate(struct fa *fa, int limit, char ***words) {
    ...
    *words = NULL;
    ...
    /* We use the visited bit to track which autstates we already visited
 ...
 * autstate s to p on character c by two transitions s(X) -> r, r(c) -> p via
 * a new autstate r.
 * If ADD_MARKER is true, also add for each original autstate s a new a loop
 * s(Y) -> q and q(X) -> s through a new autstate q.
 *
...
static struct fa *expand_alphabet(struct fa *fa, int add_marker,
    ...
    list_for_each(p, fa->initial) {
        ...
        struct autstate *r = add_autstate(fa, 0);
        r->trans = p->trans;
        r->tused = p->tused;
        r->tsize = p->tsize;
        p->trans = NULL;
        p->tused = p->tsize = 0;
        ret = add_new_auttrans(p, r, X, X);
        if (ret < 0)
            goto error;
        if (add_marker) {
            struct autstate *q = add_autstate(fa, 0);
            ret = add_new_auttrans(p, q, Y, Y);
            if (ret < 0)
                goto error;
            ret = add_new_auttrans(q, p, X, X);
            if (ret < 0)
                goto error;
        }
    }
    ...
    list_for_each(s, fa->initial) {
        ...
        for (int i=0; i < tused; i++) {
            ...
            if (t->min >= 'A' && t->max <= 'Z') {
                ...
                /* t->min < 'A' */
                t->max = 'A' - 1;
                F(add_new_auttrans(s, t->to, lc_min, lc_max));
            } else if (t->min >= 'A') {
                /* t->max > 'Z' */
                t->min = 'Z' + 1;
                F(add_new_auttrans(s, t->to, lc_min, lc_max));
                ...
                /* t->min < 'A' && t->max > 'Z' */
                F(add_new_auttrans(s, t->to, 'Z' + 1, t->max));
                s->trans[i].max = 'A' - 1;
                F(add_new_auttrans(s, s->trans[i].to, lc_min, lc_max));
            }
        }
    }
    F(collect(fa));
...
static int convert_trans_to_re(struct autstate *s) {
    struct re *re = NULL;
    size_t nto = 1;
    struct auttrans *trans = NULL;
    int r;
    ...
    struct autstate *to = s->trans[0].to;
    int tind = 0;
    for_each_trans(t, s) {
        if (t->to != to) {
            trans[tind].to = to;
...
ATTRIBUTE_RETURN_CHECK
static int add_new_re_trans(struct autstate *s1, struct autstate *s2,
                            struct re *re) {
    int r;
    r = add_new_auttrans(s1, s2, 0, 0);
    if (r < 0)
        return -1;
    last_trans(s1)->re = re;
    return 0;
...
/* Add the regular expression R1 . LOOP* . R2 to the transition
   from S1 to S2. */
static int re_collapse_trans(struct autstate *s1, struct autstate *s2,
...
static int convert_strings(struct fa *fa) {
    ...
    /* Abuse hash to count indegree, reachable to mark visited autstates */
    list_for_each(s, fa->initial) {
        s->hash = 0;
        s->reachable = 0;
    }
    ...
    for (struct autstate *s = fa->initial;
        ...
        for (int i=0; i < s->tused; i++) {
            ...
            if (! to->reachable) {
                to->reachable = 1;
                F(autstate_set_push(worklist, to));
            }
        }
    }
 ...
 * (1) For all autstates S1 and S2, convert the transitions between them
 *     into one transition whose regexp is a CSET
 * (2) Add a new initial autstate INI and a new final autstate FIN to the automaton,
 *     a transition from INI to the old initial autstate of FA, and a transition
 *     from all accepting autstates of FA to FIN; the regexp on those transitions
 *     matches only the empty word
 * (3) Eliminate autstates S (except for INI and FIN) one by one:
...
int fa_as_regexp(struct fa *fa, char **regexp, size_t *regexp_len) {
    int r;
    struct autstate *fin = NULL, *ini = NULL;
    struct re *eps = NULL;
