...
static struct lens *make_lens_binop(enum lens_tag tag, struct info *info,
    ...
    if (l1->tag == tag) {
        for (int i=0; i < l1->nchildren; i++)
            lens->children[i] = ref_(l1->children[i]);
        unref(l1, lens);
    } else {
        lens->children[0] = l1;
    }
    ...
    if (l2->tag == tag) {
        for (int i=0; i < l2->nchildren; i++)
            lens->children[n1 + i] = ref_(l2->children[i]);
        unref(l2, lens);
    } else {
        lens->children[n1] = l2;
    }
...
static struct value *make_lens_value(struct lens *lens) {
    struct value *v;
    v = make_value(V_LENS, ref_(lens->info));
    v->lens = lens;
    return v;
}
...
struct value *lns_make_plus(struct info *info, struct lens *l, int check) {
    ...
    conc = lns_make_concat(ref_(info), ref_(l), ref_(star->lens), check);
    unref(star, value);
    return conc;
}
...
struct value * lns_make_square(struct info *info, struct lens *l1,
    ...
    res = lns_make_concat(ref_(info), ref_(l1), ref_(l2), check);
    if (EXN(res))
        goto error;
    cnt1 = res;
    res = lns_make_concat(ref_(info), ref_(cnt1->lens), ref_(l3), check);
    if (EXN(res))
        goto error;
    cnt2 = res;

    sqr = make_lens_unop(L_SQUARE, ref_(info), ref_(cnt2->lens));
    ERR_NOMEM(sqr == NULL, info);

    for (int t=0; t < ntypes; t++)
        ltype(sqr, t) = ref_(ltype(cnt2->lens, t));
...
ambig_check(struct info *info, struct fa *fa1, struct fa *fa2,
    ...
    if (r < 0) {
        exn = make_exn_value(ref_(info), "not enough memory");
        if (exn != NULL) {
            return exn;
        } else {
    ...
    if (upv != NULL) {
        ...
        exn = make_exn_value(ref_(info), "%s", msg);
        if (iterated) {
...
static struct value *make_exn_square(struct info *info, struct lens *l1,
    ...
    char *fi;
    struct value *exn = make_exn_value(ref_(info), "%s",
...
static struct trans *add_trans(struct rtn *rtn, struct state *state,
    ...
    if (l != NULL) {
        result->lens = ref_(l);
        result->re = ref_(ltype(l, rtn->lens_type));
    }
 error:
    return result;
}
...
static struct regexp *rtn_reduce(struct rtn *rtn, struct lens *rec) {
    ...
    for (int i=0; i < prod->start->ntrans; i++) {
        if (prod->start->trans[i].to == prod->end) {
            ...
            result = ref_(prod->start->trans[i].re);
        }
    }
    return result;
 error:
...
static void propagate_type(struct lens *l, enum lens_type lt) {
    ...
    case L_SQUARE:
        propagate_type(l->child, lt);
        ltype(l, lt) = ref_(ltype(l->child, lt));
        break;
    default:
        BUG_LENS_TAG(l);
        break;
...
static struct info *merge_info(struct info *i1, struct info *i2) {
    ...
    info->filename = ref_(i1->filename);
...
exn_multiple_epsilons(struct lens *lens,
    ...
    exn = make_exn_value(ref_(lens->info),
...
struct value *lns_check_rec(struct info *info,
    ...
    if (! body->recursive) {
        result = make_lens_value(ref_(body));
    ...
    if (rec->atype == NULL) {
        result = make_exn_value(ref_(rec->info),
    ...
    result = lns_make_rec(ref_(rec->info));
    struct lens *top = result->lens;
    for (int t=0; t < ntypes; t++)
        ltype(top, t) = ref_(ltype(rec, t));
    top->value = rec->value;
    top->key = rec->key;
