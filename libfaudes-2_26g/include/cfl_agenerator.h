 ...
 * The TaGenerator transparently supports extended attribute semantics, buit does not provide 
...
class TaGenerator : public vGenerator {
 public:
...
TEMP const TaNameSet<EventAttr>& THIS::AlphabetTaGen(void) {
  static TaNameSet<EventAttr>* pstatic = new TaNameSet<EventAttr>();
  return *pstatic;
}
TEMP const  TaIndexSet<StateAttr>& THIS::StatesTaGen(void) {
  static  TaIndexSet<StateAttr>* pstatic = new TaIndexSet<StateAttr>();
  return *pstatic;
}
TEMP const TaTransSet<TransAttr>& THIS::TransRelTaGen(void) {
  static TaTransSet<TransAttr>* pstatic = new TaTransSet<TransAttr>();
  return *pstatic;
}
TEMP const GlobalAttr& THIS::GlobalTaGen(void) {
  static GlobalAttr* pstatic = new GlobalAttr();
  return *pstatic;
}
...
TEMP void THIS::UpdateCore(void) {
  ...
  if(tmm) { 
    ...
    errstr << "cannot cast attributes for generator " << Name();
...
TEMP THIS& THIS::Assign(const TaGenerator& rGen) {
  FD_DG("TaGenerator(" << this << ")::Assign(" << &rGen << ")");
  ...
  *pTransRel= *rGen.pTransRel;
  ...
  if(!Valid()) {
    FD_DG("TaGenerator()::Assign(): invalid generator");
    DWrite(); 
    abort();
  }
#endif
...
// Copy(gen) to other generator type 
TEMP THIS& THIS::Assign(const vGenerator& rGen) {
  FD_DG("TaGenerator(" << this << ")::Assign([v]" << &rGen << ")");
  FD_DG("TaGenerator(" << this << ")::Assign(): src type " << typeid(rGen).name());
  FD_DG("TaGenerator(" << this << ")::Assign(): dst type " << typeid(*this).name());
  // call base
  BASE::Assign(rGen);
  // report
#ifdef FAUDES_DEBUG_GENERATOR
  //  FD_DG("TaGenerator(" << this << ")::Copy([type " << typeid(rGen).name() <<"]" << &rGen << "): report");
  //  Write();
  //  rGen.Write();
#endif
  return *this;
...
// copy from other Generator (try to convert attributes)
...
TEMP TaGenerator<GlobalAttr,StateAttr,EventAttr,TransAttr>& THIS::operator= (const TaGenerator& rOtherGen) {
  FD_DG("TaGenerator(" << this << ")::operator = " << &rOtherGen);
  return Assign(rOtherGen);
}

// operator=
TEMP TaGenerator<GlobalAttr,StateAttr,EventAttr,TransAttr>& THIS::operator= (const vGenerator& rOtherGen) {
...
TEMP bool THIS::Valid(void) {
  ...
  // test types
  bool tmm=false;
  if(typeid(Alphabet().Attribute())!=typeid(const EventAttr&)) tmm=true;
  if(typeid(States().Attribute())!=typeid(const StateAttr&)) tmm=true;
  if(typeid(TransRel().Attribute())!=typeid(const TransAttr&)) tmm=true;
...
TEMP void THIS::TransAttribute(const Transition& rTrans, const Type& rAttr) {
  ...
  mpTransRel->Attribute(rTrans, *attrp);
}
...
TEMP void THIS::StateAttribute(Idx index, const Type& rAttr) {
  ...
  mpStates->Attribute(index, *attrp);
}
