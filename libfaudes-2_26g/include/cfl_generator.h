...
class vGenerator : public Type  {
 ...
 public:
   ...
   * The current implementation tries to be smart and copies attributes
   * if types can be casted. Use AssignWithoutAttributes to 
   * explicitely ignore attributes.
   *
   * @param rSrc
   *   Source to copy from.
   */
  virtual vGenerator& Assign(const vGenerator& rSrc); 
 ...
 /** 
  ...
  /**
   * Return one word accepted by this generator. Added by Chinmay
   * (chinu.pandey@gmail.com)
   *
   */

  std::string GetOneWord();

  std::string GetWord();
  /**
   * Check if this word is accepted by this generator
   * Currently it checks in deterministic generator only 
   */
