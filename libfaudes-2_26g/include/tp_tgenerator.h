...
class TtGenerator : public TcGenerator<GlobalAttr, StateAttr, EventAttr, TransAttr> {    
    public:
   /**
    * Constructor
    */
    ...
    */
     ...
     *   Other generator
     */
    virtual TtGenerator& operator= (const vGenerator& rOtherGen) {this->Assign(rOtherGen); return *this;};
...
TEMP THIS::TtGenerator(void) : BASE() {
  ...
  BASE::pGlobalAttribute->mpClockSymbolTable=ClockSet::StaticSymbolTablep();
...
TEMP void THIS::DelClocks(const ClockSet& clockset) {
  ...
  for(it=ClocksBegin(); it!=ClocksEnd(); ) {
    tmpit=it;
    it++;
    DelClock(*tmpit);
  }
...
TEMP ClockSet THIS::InactiveClocks(void) const {
  ...
  ClockSet res =  BASE::mClocks - ActiveClocks();
...
TEMP void THIS::DelInactiveClocks(void) {
  ...
  BASE::pGlobalAttribute->mClocks.Erase(iaclocks);
}
...
TEMP bool THIS::SetTransition(
  ...
  ConsistentClocks(rResets);
#endif
  BASE::TransAttribute(Transition(rX1,rEv,rX2),attr);
  return res;
}
