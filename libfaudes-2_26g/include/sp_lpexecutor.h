...
class LoggingExecutor : public ParallelExecutor {    
...
FAUDES_TYPE_DECLARATION(LogginExecutor,LoggingExecutor,ParallelExecutor)
 ...
 public:
   ...
   *
   */
   tpTime::Type CurrentTime(void) const { return ParallelExecutor::CurrentTime();} ;
  ...
  typedef enum { Statistics=0x01, States=0x02, Events=0x04, Time=0x08} LogMode;
  ...
  typedef struct {
    ...
    tpTime::Type mTime;          //// faudes time (when entered mState)
    tpTime::Type mDuration;      //// faudes duration (how long we reside mState);
