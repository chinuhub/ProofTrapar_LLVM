...
class IndexSet : public virtual TBaseSet<Idx> {
...
template<class Attr>
class TaIndexSet : public IndexSet, public TaBaseSet<Idx,Attr> {
...
// Relaxed Assign()
template<class Attr>
TaIndexSet<Attr>& TaIndexSet<Attr>::Assign(const IndexSet& rSourceSet) {
...
template<class Attr>
TaIndexSet<Attr>& TaIndexSet<Attr>::operator=(const IndexSet& rSourceSet) {
...
// Attributes(set)
template<class Attr>
void TaIndexSet<Attr>::Attributes(const TaIndexSet& rOtherSet) {
  FD_DC("TaIndexSet(" << this << ")::Attributes(set) with type " << typeid(rOtherSet.Attribute()).name());
  TaBaseSet<Idx,Attr>::Attributes(rOtherSet);
}

// Attributes(set)
template<class Attr>
void TaIndexSet<Attr>::Attributes(const IndexSet& rOtherSet) {
  FD_DC("TaIndexSet(" << this << ")::Attributes(set) with type " << typeid(rOtherSet.Attribute()).name());
  TaBaseSet<Idx,Attr>::Attributes(rOtherSet);
}
...
//Clear
template<class Attr>
void TaIndexSet<Attr>::Clear(void) {
  FD_DC("TaIndexSet(" << this << ")::Clear()");
  TaBaseSet<Idx,Attr>::Clear();
}  
...
Idx TaIndexSet<Attr>::Insert(void) {
  ...
  TaBaseSet<Idx,Attr>::Insert(index);
  return index;
}
...
bool TaIndexSet<Attr>::Insert(const Idx& rIndex) {
  ...
  bool ret=IndexSet::Insert(rIndex); // automatic: keep attribute if exists
  return ret;
}
...
// InsertSet(set)
template<class Attr>
void TaIndexSet<Attr>::InsertSet(const TaIndexSet& rOtherSet) {
  FD_DC("TaIndexSet(" << this << ")::InsertSet( T " << &rOtherSet << ")");
