...
class DeviceExecutor : public ProposingExecutor {  
...
FAUDES_TYPE_DECLARATION(DeviceExecutor,DeviceExecutor,LoggingExecutor)
 ...
 /**
   ...
   *
   */
  void ToleranceTime(tpTime::Type maxgap) {mMaxSyncGap=maxgap; mSyncMode |= SyncStrictTime;};
   ...
   *
   * @param duration
   *   Max duration of execution wrt generator time, tpTime::Max for unlimited
   * @return
   ...
   *
   * @param duration
   *   Duration of execution wrt generator time, tpTime::Max for infinite
   * @return
   *  True, for no error
   */
